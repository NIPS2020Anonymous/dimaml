...
from .utils import straight_through_grad, copy_and_replace, NONE_TENSOR, is_none_tensor
...
class IngraphMomentum(IngraphGradientDescent):
    ...
    def __init__(self, learning_rate=None, momentum=None, weight_decay=None,
        ...
        self.hparams = dict(learning_rate=learning_rate, momentum=momentum, weight_decay=weight_decay)
...
class IngraphRMSprop(IngraphGradientDescent):
    OptimizerState = namedtuple(
        "OptimizerState", ["grad_momenta", "ewma_grad_norms_sq", "learning_rate", "momentum", "alpha", "epsilon"])

    def __init__(self, learning_rate=None, log_learning_rate=None, momentum=None, alpha=None,
        ...
        :param momentum: momentum coefficient, the update direction is (1 - momentum) * prev_update  + update,
            default = no momentum
        :param alpha: RMSProp decay coefficient, the update is scaled by 1 / sqrt(ewma + epsilon)
            where ewma = prev_ewma * alpha + dL/dw ^ 2 * (1 - alpha), default = no RMSProp
        :param force_trainable_params: if True, treats all optimizer parameters that are not None as learnable
            parameters that are trained alongside other non-edited layers
        """
        nn.Module.__init__(self)
        self.hparams = dict(
            ...
            momentum=momentum, alpha=alpha, epsilon=epsilon, log_epsilon=log_epsilon
        )
    ...
    def get_initial_state(self, module: nn.Module, *, parameters: list, **overrides):
        ...
        * the optimizer state at the input of :step: method forces requires_grad=True to all tensors.
        ...
        if alpha is not None:
            alpha = straight_through_grad(torch.clamp, min=0.0, max=1.0)(torch.as_tensor(alpha))
        ...
        dummy_ewma = [torch.zeros_like(params, requires_grad=False) for params in
                      parameters]

        return self.OptimizerState(dummy_grad_momenta, dummy_ewma, learning_rate, momentum, alpha, epsilon)
    ...
    def step(self, state: OptimizerState, module: nn.Module, loss, parameters=None, detach=False, **kwargs):
        ...
        :returns: new_state, updated_self
            ...
            updated_self: updated copy of module
        """
        grad_momenta, ewma_grad_norms_sq, learning_rate, momentum, alpha, epsilon = state
        learning_rate, momentum, alpha, epsilon = [
            tensor.to(loss.device) for tensor in (learning_rate, momentum, alpha, epsilon)]
        parameters = list(parameters or module.parameters())
        gradients = torch.autograd.grad(loss, parameters, create_graph=not detach,
                                        only_inputs=True, allow_unused=False)
        updates = list(gradients)  # updates are the scaled/accumulated/tuned gradients

        if not is_none_tensor(alpha):
            for i in range(len(ewma_grad_norms_sq)):
                ewma_grad_norms_sq[i] = alpha * ewma_grad_norms_sq[i] + (1. - alpha) * gradients[i] ** 2
                ...
                updates[i] = grad_momenta[i] / (torch.sqrt(ewma_grad_norms_sq[i])  + epsilon)

        if not is_none_tensor(momentum) and float(momentum) != 0:
            for i in range(len(grad_momenta)):
                grad_momenta[i] = grad_momenta[i] * momentum + updates[i]
            updates = grad_momenta

        # finally, perform sgd update
        updated_module = get_updated_model(module, loss=None, gradients=updates, parameters=parameters,
                                           learning_rate=learning_rate, **kwargs)
        new_state = self.OptimizerState(grad_momenta, ewma_grad_norms_sq, learning_rate, momentum, alpha, epsilon)
...
class IngraphAdam(IngraphGradientDescent):
    ...
    def __init__(self, learning_rate=None, beta1=None, beta2=None, epsilon=None,
                 weight_decay=None, force_trainable_params=False):
        """
        Ingraph optimzier that performs RMSProp updates with optional momentum
        :param learning_rate: log(alpha) for gradient descent, all updates are scaled by exponent of this value
        :param momentum: momentum coefficient, the update direction is (1 - momentum) * prev_update  + update,
        ...
        """
        nn.Module.__init__(self)
        self.hparams = dict(learning_rate=learning_rate, beta1=beta1, beta2=beta2,
